<template>
  <div v-if="participant" :key="participant?.id" class="col-12 score-card d-flex justify-space-evenly align-items-center text-white my-0 pe-2 py-2 border">
    ...
    <span class="col-3 text-center">Challenges Created: {{ createdChallengesCount }}</span>
...
export default {
  ...
  setup(props) {
    return {
      createdChallengesCount: computed(() => {
        const isCreator = AppState.AccountState.participation.filter(p => p.accountId == props.participant.accountId && p.creator == true)
        return isCreator.length
      })
    }
