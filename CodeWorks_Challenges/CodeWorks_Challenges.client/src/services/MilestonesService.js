...
class MilestonesService {
  ...
  async createMilestone(milestoneData) {
    ...
    AppState.AccountState.milestones.push(new Milestone(res.data))
  }
  async getMilestones() {
    const res = await api.get('api/milestones')
    logger.log('[getMilestones]', res.data)
    AppState.AccountState.milestones = res.data.map(m => new Milestone(m))
  ...
  async removeMilestone(milestoneId) {
    ...
    AppState.AccountState.milestones.splice(removeMilestone)
  }
