...
export default {
  ...
  setup(props) {
    ...
    const currentRankBadge = computed(() => {
      let lastKey = 0
      for (const badge of RANK_VALUES) {
        if (props.profile.rank >= badge.RANK_THRESHOLD) {
          lastKey = badge.RANK_THRESHOLD
        }
      }
      return RANK_VALUES.find(badge => badge.RANK_THRESHOLD === lastKey)
    })

    const rankBadgePercentage = computed(() => {
      const rankThreshold = Object.values(RANK_VALUES).map(badge => badge.RANK_THRESHOLD);
    ...
    const nextRankBadge = computed(() => {
      ...
      return RANK_VALUES.find(badge => badge.RANK_THRESHOLD === nextRank);
    });
